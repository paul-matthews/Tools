...
def action(region, variant, chord, device_bits):
    ...
    return {
        ...
        "Hot Key": hot_key(region["chord"], control_id,
...
def cheat_sheet(entries, chord, width, height):
    ...
    lines = [
        ...
        "either hand can drive it alone while the other stays on the mouse. "
        "After **⌥`** (Moom's keyboard controller) the chord key alone does the "
        "same thing — those are the left-hand keys.",
        "",
    ]
    ...
    lines += [
        ...
        "action either way. The chord is always a letter or a digit — never "
    ...
    for region in entries:
        keycode = spec.KEYS[region["chord"]][1]
        lines.append(f"| {region['title']} | `{region['left']}` | "
                     f"`{region['right']}` | `{qmk}({keycode})` |")
    lines += [
        "",
